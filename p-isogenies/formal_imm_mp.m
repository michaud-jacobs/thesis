...
// (if m = 2 and p = 23 then a formal immersion criterion will fail for a pair of cusps, this is since X_0(46) is hyperelliptic)
...
check_formal_immersion := function(m,p);
    N := m*p;
    // We first choose the cusp forms which correspond to abelian varieties with rank 0 over Q
    S:=CuspForms(N);
    R<q>:=PowerSeriesRing(Integers());
    J:=JZero(N); // modular abelian variety J_0(N)
    dec := Decomposition(J); // isogeny decomposition over Q into Q-simple abelian varieties
    Nfs := [* *];  // build up to list of cusp forms
    for d in dec do
    ...
    for f in Nfs do
        ...
        RK:= ChangeRing(R,K); // power series in q over K
        SK:= BaseChange(S,K); // space of cusp forms over K
        ...
        fK:= SK ! (RK ! fexp); // cusp form viewed as an element of space of cusp forms over K
        OK := Integers(K);
        if Degree(K) eq 1 then // Rational field
            int_bas := Basis(OK); 
        else
            int_bas := Basis(Codifferent(1*OK)); // integral basis of the codifferent
        end if;
        for b in int_bas do
            bfK := b*fK;
            tr_bfK := S ! (R ! (&+[Trace(Coefficient(bfK,i))*q^i : i in [1..prec-1]] + O(q^(prec-1)))); // new basis element
    ...
    for i in [1,2,3,4] do  // We now form the formal immersion matrices
        ...
        Col1 := [Integers() ! Coefficient(f,1) : f in CuspExps[1]]; // first column of formal immersion matrix
        // second column is defined according to whether we have a double cusp or not
        if i eq 1 then // double cusp
            Col2 := [Integers() ! Coefficient(f,2) : f in CuspExps[1]];
        else           // pair of distinct cusps
