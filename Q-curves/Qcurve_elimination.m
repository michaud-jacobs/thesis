...
for q in [17,41,89,97] do
    ...
    print "The Galois conjugacy class sizes are:", class_sizes;
    ...
    if q eq 97 then
        print "Eliminating final two newforms using alternative method";
        for i in [28,29] do
            g := Nfreps[i];
    ...
    if q eq 41 then
        ...
        for x,m in [0..p-1] do
            ...
            t2 := <RatTracexmp(x,m,p,2),x,m>;
            assert t1 eq t2; // Independent of cs
            if t1[1] eq -4 then
                assert x eq 6; // only x = 6 gives a trace of -4.
            end if;
        end for;

        // We have
        for m in [0..p-2] do
            assert QTracexmp(6,1,p,1) eq 6; // when x is 6 both traces are 6
    ...
    // Sanity check:
    // We check for q = 17 and q = 89 that when we enter the true solution (-23)^2 - 17 = 2^9 or (-91)^2 - 89 = 2^13
